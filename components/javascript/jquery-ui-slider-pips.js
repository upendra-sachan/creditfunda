        ...
        pips: function(settings) {
            ...
            function labelClick(label) {
                ...
                if (true === slider.options.range) {
                    ...
                    slider_id = slider_id.split('_');
                    $('#slider_'+slider_id[1]+'_txt').val(NewMoneyFormatInr(val));
                    $("#").val(NewMoneyFormatInr(val));
                    $("#slider_"+slider_id[1]+"_inword").html(digitToWordConvertor(val));                    
                }
            }
...
//                    }
                    ...
                    if(percent == '25.0000%' || percent == '50.0000%' || percent == '75.0000%'){
                        slider_block = 'slider-block';
                        label_class = 'label-block';
                    }
                ...
                collection +=
                        "<span class=\"" + classes +" "+ slider_block + "\" style=\"" + css + "\">" +
                        "<span class=\"ui-slider-line\"></span>" +
                        "<span class=\"ui-slider-label "+label_class+"\" data-value=\"" + labelValue + "\">" + options.formatLabel(NewMoneyFormatInr(label)) + "</span>" +
                        "</span>";
        ...
        float: function(settings) {
            ...
            if (options.handle) {
                ...
                if (slider.options.values) {
                    ...
                    $tip = [
                        $("<span class=\"ui-slider-tip\">" + options.formatLabel(NewMoneyFormatInr(vals[0])) + "</span>"),
                        $("<span class=\"ui-slider-tip\">" + NewMoneyFormatInr(options.formatLabel(NewMoneyFormatInr(vals[1]))) + "</span>")
                    ];
                    ...
                    $tip = $("<span class=\"ui-slider-tip\">" + options.formatLabel(NewMoneyFormatInr(val)) + "</span>");
            ...
            slider.element.on(options.event, function(e, ui) {
                ...
                $(ui.handle).find(".ui-slider-tip").html(options.formatLabel(NewMoneyFormatInr(val)));
...
function NewMoneyFormatInr(amount) {
    var result = null;
    amount = amount.toString().replace(/,/g, '');
    if (amount > 0) {
        //var amount=12345652457.557;
...
function getIntValueFromInr(value) {
    return value.replace(/,/g, '');
}


function changeSliderValue(event, object, quote_element_object) {    
    $object_split_Array = $(object).attr('id').split('_');
    var slider_id = $object_split_Array[0] + "_" + $object_split_Array[1];

    iconGroupButtonAction.SetValueByValueAttr($(object), quote_element_object);
    var value = getIntValueFromInr($(object).val());
    if(value > 0 ){
        var finalvalue = parseInt(getIntValueFromInr($(object).val()));
        $("#" + slider_id).slider('value', finalvalue);
    } else {
        $("#" + slider_id).slider('value', $("#" + slider_id).attr("min"));
    }
    var display_value = NewMoneyFormatInr($(object).val());
    if (display_value == 0) {
        display_value = "";
    }
    $(object).val(display_value);
}